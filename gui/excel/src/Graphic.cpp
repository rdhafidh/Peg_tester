...
void Graphic::mouseMoveEvent(QMouseEvent *event)
{
   if (!event->buttons())
   {
      ...
      if (position == TopLeft || position == BottomRight){
         ...
         qDebug()<<"pos:"<<event->pos ()<<"parent"<<mapToParent(event->pos ())<<" pos item to parent"<<mapToParent(pos());
...
void Graphic::moveManual(int x, int y)
{
   emit somethingChangedSignal();
    QWidget::move(x,y);
}

void Graphic::moveManual(const QPoint &point)
{
    somethingChangedSignal();
  QWidget::move(point);
 //  emit somethingChangedSignal();
}
...
void Graphic::drawBoundary(QPainter &painter)
{
   ...
   {
      QBrush brush(QColor::fromRgb (228,228,228));
...
QPoint Graphic::getPosisiDiTable() const
{
    return mapToParent(pos());
}
...
void Graphic::out(QDataStream &outStream) const
{
   ...
   outStream << getPosisiDiTable ()<< qint16(scaleSize.width()) << qint16(scaleSize.height());
}
