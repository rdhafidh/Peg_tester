...
QVariant CalculatorPEG::hitung(Table *t, int row, int col) {
  ...
  try {
    QVariant var= EvalBase::eval(ast, table, row, col);
    switch (var.type()) {
    case QVariant::Double:
        return EvalBase::toNumberFromVariant (var);
    default:
        return var;
    }
...
QVariant CalculatorPEG::hitung(const QString &formulas)
{
    ...
    ast = peg::AstOptimizer(true).optimize(ast);
    try {
        QVariant var= EvalBase::eval(ast, nullptr, 0, 0);
#ifdef ASKDEBUG
qDebug()<<"end var "<<var;
#endif
        switch (var.type()) {
