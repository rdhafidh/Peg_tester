...
QAction *MainWindow::getUndo()
{
    return undoAction;  
}

QAction *MainWindow::getRedo()
{
    return redoAction;
}

QAction *MainWindow::getAlignLeft()
{
    return alignLeftAction;
}

QAction *MainWindow::getAlignRight()
{
    return alignCenterAction;
}

QAction *MainWindow::getAlignCenter()
{
    return alignRightAction;
}
MainWindow::MainWindow(QWidget *parent):QMainWindow(parent,Qt::Widget)
{
   findDialog = 0;
...
void MainWindow::updateStatusBar()
{
   ...
   if (!CalculatorPEG::isformula (table -> getCurrentItemText(Qt::EditRole)))
      shownText += "0";
   else 
      shownText += table -> getCurrentItemText();
   
   this->askUpdateCurrentLabelCell (shownText);
}
...
bool MainWindow::askToSaveChanges()
{
   if (isWindowModified())
   {
      int answer = QMessageBox::warning(this, "Save", 
                                        "Dokumen ini telah dimodifikasi.\n"
...
bool MainWindow::open(QString fileName)
{
   if (fileName.isEmpty())
      fileName = QFileDialog::getOpenFileName(this, tr("Open"), ".",
                                                    tr("Simulator Spreadsheet Documents (*.qsd)\n"
                                                      "Semua File (*.*)"));
   if (!fileName.isEmpty())
   {
...
{
    this->alignLeftAction->setChecked (true);
    this->alignCenterAction->setChecked (false);
    this->alignRightAction->setChecked (false);
    align(true);
}

void MainWindow::alignright()
{
    this->alignLeftAction->setChecked (false);
    this->alignCenterAction->setChecked (false);
    this->alignRightAction->setChecked (true);
    align(true);
}
...
{
    this->alignLeftAction->setChecked (false);
    this->alignCenterAction->setChecked (true);
    this->alignRightAction->setChecked (false);
    align(true);
}
